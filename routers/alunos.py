...
def listar_alunos(session: Annotated[Session, Depends(get_db)], ativo: bool | None = None, limite: int = 10):
...
def buscar_aluno(session: Annotated[Session, Depends(get_db)], aluno_id: int):
...
def criar_aluno(session: Annotated[Session, Depends(get_db)], dados: AlunoEntrada):
...
def criar_alunos_em_lote(session: Annotated[Session, Depends(get_db)], curso_id: int, dados: MatriculaEmLote):
    curso_existe = obter_ou_404(session, Curso, curso_id, "Curso")
    alunos = [Aluno(**a.model_dump(), curso_id=curso_id) for a in dados.alunos]
    for aluno in alunos:
        aluno.cursos.append(curso_existe)
...
def atualizar_aluno(session: Annotated[Session, Depends(get_db)], aluno_id: int, dados: AlunoEntrada):
    aluno = session.get(Aluno, aluno_id)
    if aluno is None:
...
def alterar_aluno(session: Annotated[Session, Depends(get_db)], aluno_id: int, dados: AlunoPatch):
    aluno = session.get(Aluno, aluno_id)
    if aluno is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='Aluno não encontrado!')
...
def remover_aluno(session: Annotated[Session, Depends(get_db)], aluno_id: int):
    aluno = session.get(Aluno, aluno_id)
    if aluno is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='Aluno não encontrado!')
